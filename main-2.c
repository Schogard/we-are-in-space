...
int main()
{
    ...
    {
        histo[Pixmap[i]]++; //generating histogramm
    }
    ...
    //Generating Traces.txt
    FILE *fout;
    fout = fopen("/Users/manuel/Documents/Programming_C/Project C ba3/Traces.txt", "w"); //this one in write mode
    ...
    if(histo[colour_control_points]==4)
    {
        ...
        for(i=0; i<hauteur*largeur; i++) //generating the coordinates for controlpoints
        {
            ...
            controlpointx= i%largeur-1;
            controlpointy= i/largeur;
    ...
    for(k=0; k<4; k++)//the first 5 colours
    {
        if(colours[k]!=0)
        {
            fprintf(fout, "C%d= [\n", k);
